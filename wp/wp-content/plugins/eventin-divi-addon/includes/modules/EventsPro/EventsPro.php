...
class DIVI_Eventspro extends ET_Builder_Module {

	public $slug       = 'eventin_events_pro';
	...
	public function init() {
		$this->name = esc_html__( 'Eventin Events Pro', 'eventin-divi-addon' );
	}
	...
	public function get_fields() {
		return array(
			'etn_event_style'   => array(
				...
				'options'          => array(
					'style-1' => esc_html__( 'Event 1', 'eventin-divi-addon' )
				),
			...
			'etn_event_count'   => array(
				...
				'type'            => 'text',
				'option_category' => 'basic_option',
				'description'     => esc_html__( 'Define the number of events that should be displayed per page. Leave blank or enter -1 to show all events.', 'eventin-divi-addon' ),
				'depends_show_if' => 'off',
				'toggle_slug'     => 'main_content',
			),
			'etn_event_col'   => array(
			...
			'show_attendee_count' => array(
				'label'       => esc_html__( 'Show Attendee Count', 'eventin-divi-addon' ),
				'type'        => 'yes_no_button',
				'default'     => 'off',
				'options'     => array(
					'on'  => esc_html__( 'Yes', 'eventin-divi-addon' ),
			...
			'__all_events' => array(
				'type' => 'computed',
				'computed_callback' => array( 'DIVI_Eventspro', 'get_eventin_events' ),
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		// If event_count is blank, set to -1 to show all events
		...
		$show_attendee_count = ($this->props['show_attendee_count'] == 'on')? 'yes': 'no';
		...
		$shortcode_content = do_shortcode("[etn_pro_events_standard style ='{$style}' event_cat_ids = '{$category_terms}' event_tag_ids = '{$tag_terms}' orderby='{$orderby}' order='{$order}' filter_with_status='{$filter_status}' event_col ='{$event_col}' show_desc='{$show_desc}' desc_limit='{$etn_desc_limit}' show_attendee_count='{$show_attendee_count}' show_btn='{$show_btn}' show_thumb='{$show_thumb}' show_category='{$show_category}' event_count = '{$event_count}' show_parent_event='{$show_parent_event}' show_child_event='{$show_child_event}' ]");
	...
	static function get_eventin_events( $agrs = array() ) {
		...
		$show_attendee_count = ($agrs['show_attendee_count'] == 'on')? 'yes': 'no';
		...
		echo do_shortcode("[etn_pro_events_standard style ='{$style}' event_cat_ids = '{$category_terms}' event_tag_ids = '{$tag_terms}' orderby='{$orderby}' order='{$order}' filter_with_status='{$filter_status}' event_col ='{$event_col}' show_desc='{$show_desc}' desc_limit='{$etn_desc_limit}' show_attendee_count='{$show_attendee_count}' show_btn='{$show_btn}' show_thumb='{$show_thumb}' show_category='{$show_category}' event_count = '{$event_count}'  show_parent_event='{$show_parent_event}' show_child_event='{$show_child_event}' ]");
