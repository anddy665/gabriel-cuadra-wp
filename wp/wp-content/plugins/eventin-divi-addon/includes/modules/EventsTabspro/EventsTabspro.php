...
class DIVI_EventsTabspro extends ET_Builder_Module {

	public $slug       = 'eventin_events_tabs_pro';
	...
	public function init() {
		$this->name = esc_html__( 'Eventin Events Tabs Pro', 'eventin-divi-addon' );
	}
		...
		return array(
			'etn_event_style'   => array(
				...
				'options'          => array(
					...
					'event-3'  => esc_html__( 'Event 3', 'eventin-divi-addon' ),
					'event-4'  => esc_html__( 'Event 4', 'eventin-divi-addon' ),
				),
				'default'         => 'event-1',
			...
			'__all_events' => array(
				'type' => 'computed',
				'computed_callback' => array( 'DIVI_EventsTabspro', 'get_eventin_events' ),
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		$shortcode_content = do_shortcode("[etn_pro_events_tab style ='{$style}' event_cat_ids = '{$category_terms}' event_tag_ids = '{$tag_terms}' order='{$order}' orderby='{$orderby}' filter_with_status='{$filter_status}' event_col ='{$event_col}' show_desc='{$show_desc}' desc_limit = {$etn_desc_limit} show_attendee_count='{$show_attendee_count}' show_btn='{$show_btn}' show_thumb='{$show_thumb}' show_category='{$show_category}' event_count = '{$event_count}']");
	...
	static function get_eventin_events( $agrs = array() ) {
		...
		echo do_shortcode("[etn_pro_events_tab style ='{$style}' event_cat_ids = '{$category_terms}' event_tag_ids = '{$tag_terms}' order='{$order}' orderby='{$orderby}' filter_with_status='{$filter_status}' event_col ='{$event_col}' show_desc='{$show_desc}' desc_limit = {$etn_desc_limit} show_attendee_count='{$show_attendee_count}' show_btn='{$show_btn}' show_thumb='{$show_thumb}' show_category='{$show_category}' event_count = '{$event_count}']");
